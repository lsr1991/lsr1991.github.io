...
本文阐述上传数据至HBase进行负载均衡的策略，以期达到最大的上传速率，主要为写多读少的业务服务。
...
Hash技术的应用可以有两个，一个是快速查找，另一个是均匀分布。不同应用要关注的点不同。
...
均匀分布的目标是将一个元素集合中的元素（无重复）均匀分配到固定数目的多个地址。这里关注的不是一一映射防止冲突的问题，而是如何将元素均匀映射。这取决于元素集合的分布以及哈希函数的选择。如果元素集合是均匀分布（如电话号码等），那么常用的哈希函数是取模，即z mod n，z为元素，n为散列值集合的大小。但这种函数的缺点在于当n越来越接近元素集合大小N时，散列值的分布将越来越不均匀。（参考维基百科[哈希函数](http://en.wikipedia.org/wiki/Hash_function#Hashing_uniformly_distributed_data)）根据元素集合的不同，要选择合适的哈希函数，在[哈希函数结果的分布](http://michiel.buddingh.eu/distribution-of-hash-values#summary)一文中对这些函数的结果分布根据输入数据集的不同做了对比（看不懂），但可以看出大多数hash函数的输出都是接近均匀分布的。

在redis数据库技术中，它的实现是使用crc16(key) mod 16384来将key均匀映射到16384个slot上。

均匀分布其实也是快速查找的一个需求，因为可以避免过多的冲突。

## 5.均衡负载

### 5.1.方案1

在建表时预定义region，region数目与regionserver相同，然后将散列值集合为region数目的N倍（N=1,2,3,...）均匀分配给各个region，如散列值集合为0～15，region总数为8，则可以按照0~1，2～3，4～5将散列值区间依次分配给8个region。在对HBase进行插入操作时，可以把记录中的某个特征用来计算散列值。计算出散列值之后将其添加在原始rowkey的前面组成新的rowkey，这样，散列值为1的记录就会被插入到0～1区间的region了，从而负载会均摊到各个regionserver。随着region的拆分和迁移，写负载仍然会近似均匀地分布在所有regionserver。
...
这样会有一个明显的缺点，在上传时，客户端需要跟N个regionserver保持连接，在查询时，无论是连续区域查询，还是单条查询，都需要访问N个逻辑分区，这意味着与N个regionserver都要建立连接。当集群规模较大（即N较大）时，连接开销是巨大的。因此，这种方案只适用于小规模集群（N<20）。在小集群情况下，直接让客户端遍历所有分区时，如果采取多线程查询，启动线程数为k（k<=N），不同线程的查询负载会落到不同regionserver上。也就是说，原本一个regionserver执行一次操作就能完成的查询，采取方案后每个regionserver都要执行一次操作才能完成。
